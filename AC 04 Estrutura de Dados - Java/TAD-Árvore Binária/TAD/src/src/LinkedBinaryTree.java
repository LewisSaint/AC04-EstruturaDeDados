...
//-----------------------------------------Implementaçao-----------------------------------------//
    ...
    public String binaryPreorder(BinaryTree<E> Tree, BTPosition v) {
        String finalString = v.element().toString();

        if (Tree.hasRight(v)) {
            binaryPreorder(Tree, v.getRight());
        }

        else if (Tree.hasLeft(v)) {
            binaryPreorder(Tree, v.getLeft());
        }
    ...
    public String binaryPostorder(BinaryTree<E> Tree, BTPosition v, String strInput) {
        if (Tree.hasRight(v)) {
            binaryPostorder(Tree, v.getRight(), strInput);
        }
        else if (Tree.hasLeft(v)) {
            binaryPostorder(Tree, v.getLeft(), strInput);
        }
        strInput += v.element().toString();
    ...
    public int evaluateExpression(BinaryTree Tree, BTPosition v) {



        if (Tree.isInternal(v)) {
            String o = v.element().toString();
            int x = evaluateExpression(Tree, v.getLeft());
            int y = evaluateExpression(Tree, v.getRight());
        ...
        else {return Integer.parseInt(v.element().toString());}
    }


    public String inOrder(BinaryTree Tree, BTNode v, String strInput) {
        if(Tree.hasLeft(v)) {
            inOrder(Tree, (BTNode) v.getLeft(), strInput);
        }
        ...
        if (Tree.hasRight(v)) {
            inOrder(Tree, (BTNode)v.getRight(), strInput);
        }

        return strInput;
    }

    public String printExpression(BinaryTree Tree, BTNode v, String strInput) {
        if (Tree.isInternal(v)) {strInput += "(";}
        else if (Tree.hasLeft(v)) {printExpression(Tree, (BTNode) Tree.left(v), strInput);}
        else if (Tree.isInternal(v)) {strInput = v.element().toString();}
        else {strInput = v.element().toString();}

        if (Tree.hasRight(v)) {printExpression(Tree, (BTNode) Tree.right(v), strInput);}
        else if (Tree.isInternal(v)) {strInput += ")";}

        return strInput;
    }


    public String eulerTour (LinkedBinaryTree Tree, BTNode v, String strInput) {
        strInput += v.element().toString();
        if (Tree.hasLeft(v)) {
            eulerTour(Tree, (BTNode) v.getLeft(), strInput);
            strInput += v.element().toString();
        }

        if(Tree.hasRight(v)) {
            eulerTour(Tree, (BTNode) v.getRight(), strInput);
            strInput += v.element().toString();
        }
